...
export default {
  ...
  data() {
    return {
      ...
      actionInfo: {
        ...
        modalTitle: this.departmentId ? "添加成员至部门" : "添加成员至组织",
      },
      linkInfo: {
        modalStatus: false,
        confirmLoading: false,
  ...
  created() {
    if (this.departmentId) {
      this.searching = true;
      searchInviteMember(this.keyword, this.departmentId).then((res) => {
  ...
  methods: {
    invite(item, index) {
      let app = this;
      console.log("invite", item, index);
      inviteMember(item.id, this.departmentId).then((res) => {
